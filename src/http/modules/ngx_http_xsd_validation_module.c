...
} ngx_http_xsd_validation_main_conf_t;
...
} ngx_http_xsd_validation_loc_conf_t;
...
} ngx_http_xsd_validation_ctx_t;


static ngx_int_t ngx_http_xslt_send(ngx_http_request_t *r,
    ngx_http_xsd_validation_ctx_t *ctx, ngx_buf_t *b);
...
static ngx_buf_t *ngx_http_xslt_apply_stylesheet(ngx_http_request_t *r,
    ngx_http_xsd_validation_ctx_t *ctx);
static ngx_int_t ngx_http_xslt_params(ngx_http_request_t *r,
    ngx_http_xsd_validation_ctx_t *ctx, ngx_array_t *params, ngx_uint_t final);
...
static void *ngx_http_xsd_validation_create_main_conf(ngx_conf_t *cf);
static void *ngx_http_xsd_validation_create_conf(ngx_conf_t *cf);
static char *ngx_http_xsd_validation_merge_conf(ngx_conf_t *cf, void *parent,
    void *child);
static ngx_int_t ngx_http_xsd_validation_preconfiguration(ngx_conf_t *cf);
static ngx_int_t ngx_http_xsd_validation_init(ngx_conf_t *cf);
static void ngx_http_xsd_validation_exit(ngx_cycle_t *cycle);
...
static ngx_command_t  ngx_http_xsd_validation_commands[] = {
    ...
    { ngx_string("xslt_types"),
      ...
      ngx_http_types_slot,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_xsd_validation_loc_conf_t, types_keys),
    ...
    { ngx_string("xslt_last_modified"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_xsd_validation_loc_conf_t, last_modified),
      NULL },
...
static ngx_http_module_t  ngx_http_xsd_validation_module_ctx = {
    ngx_http_xsd_validation_preconfiguration, /* preconfiguration */
    ngx_http_xsd_validation_init,             /* postconfiguration */

    ngx_http_xsd_validation_create_main_conf, /* create main configuration */
    NULL,                                  /* init main configuration */

    NULL,                                  /* create server configuration */
    NULL,                                  /* merge server configuration */

    ngx_http_xsd_validation_create_conf,      /* create location configuration */
    ngx_http_xsd_validation_merge_conf        /* merge location configuration */
};


ngx_module_t  ngx_http_xsd_validation_module = {
    NGX_MODULE_V1,
    &ngx_http_xsd_validation_module_ctx,      /* module context */
    ngx_http_xsd_validation_commands,         /* module directives */
    ...
    NULL,                                  /* init thread */
    NULL,                                  /* exit thread */
    ngx_http_xsd_validation_exit,             /* exit process */
    ngx_http_xsd_validation_exit,             /* exit master */
...
{
    ngx_http_xsd_validation_ctx_t       *ctx;
    ngx_http_xsd_validation_loc_conf_t  *conf;
    ...
    conf = ngx_http_get_module_loc_conf(r, ngx_http_xsd_validation_module);
    ...
    ctx = ngx_http_get_module_ctx(r, ngx_http_xsd_validation_module);

    if (ctx) {
        return ngx_http_next_header_filter(r);
    }

    ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_xsd_validation_ctx_t));
    if (ctx == NULL) {
        return NGX_ERROR;
    }

    ngx_http_set_ctx(r, ctx, ngx_http_xsd_validation_module);
...
static ngx_int_t
ngx_http_xslt_send(ngx_http_request_t *r, ngx_http_xsd_validation_ctx_t *ctx,
    ngx_buf_t *b)
{
    ngx_int_t                         rc;
    ngx_chain_t                       out;
    ...
    if (b == NULL) {
        return ngx_http_filter_finalize_request(r, &ngx_http_xsd_validation_module,
...
static ngx_int_t
ngx_http_xslt_add_chunk(ngx_http_request_t *r, ngx_http_xsd_validation_ctx_t *ctx,
    ngx_buf_t *b)
{
    int               err;
    xmlParserCtxtPtr  ctxt;
...
static ngx_int_t
ngx_http_xslt_params(ngx_http_request_t *r, ngx_http_xsd_validation_ctx_t *ctx,
...
{
    ngx_http_xsd_validation_loc_conf_t *xlcf = conf;

    ngx_str_t                         *value;
    ngx_uint_t                         i;
    ngx_pool_cleanup_t                *cln;
    ngx_http_xslt_file_t              *file;
    ngx_http_xsd_validation_main_conf_t  *xmcf;
    ...
    xmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_xsd_validation_module);
...
static void *
ngx_http_xsd_validation_create_main_conf(ngx_conf_t *cf)
{
    ngx_http_xsd_validation_main_conf_t  *conf;

    conf = ngx_palloc(cf->pool, sizeof(ngx_http_xsd_validation_main_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static void *
ngx_http_xsd_validation_create_conf(ngx_conf_t *cf)
{
    ngx_http_xsd_validation_loc_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_xsd_validation_loc_conf_t));
    if (conf == NULL) {
        return NULL;
    }
...
static char *
ngx_http_xsd_validation_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_xsd_validation_loc_conf_t *prev = parent;
    ngx_http_xsd_validation_loc_conf_t *conf = child;
...
static ngx_int_t
ngx_http_xsd_validation_preconfiguration(ngx_conf_t *cf)
{
    xmlInitParser();
...
static ngx_int_t
ngx_http_xsd_validation_init(ngx_conf_t *cf)
{
...
static void
ngx_http_xsd_validation_exit(ngx_cycle_t *cycle)
{
    xsltCleanupGlobals();
    xmlCleanupParser();
}
